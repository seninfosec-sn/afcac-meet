...
const SECRET = new TextEncoder().encode(
  process.env.JWT_SECRET || "resameet-super-secret-jwt-key-change-in-production-2026"
);
...
export async function POST(req: NextRequest) {
  ...
  // Read the OTP cookie
  const otpToken = req.cookies.get("resameet_otp")?.value;
  ...
  // Set session cookie
  response.cookies.set("resameet_session", sessionToken, {
    httpOnly: true,
  ...
  // Clear OTP cookie
  response.cookies.set("resameet_otp", "", {
    httpOnly: true,
    path: "/",
    maxAge: 0,
  });
