...
type AdminFilterKey = "all" | "bilateral" | "confirmed" | "pending" | "proposed" | "cancelled";
...
export default function AdminPage() {
  ...
  const [filter, setFilter]   = useState<AdminFilterKey | string>("all");
  ...
  async function handleDeleteUser(id: string, label: string) {
    if (!window.confirm(t.admin.confirmDeleteUser(label))) return;
    setUserAction(id);
  ...
  async function handleDeleteReservation(id: string, type: "bilateral" | "salle", title: string) {
    if (!window.confirm(t.admin.confirmDelete(title))) return;
    setDeleting(id);
  ...
  async function handleDeleteAllReservations() {
    if (!window.confirm(t.admin.confirmDeleteAll)) return;
    setDeleting("all");
  ...
  async function handleDeleteRoom(room: AdminRoom) {
    if (!window.confirm(t.admin.confirmDeleteRoom(room.name))) return;
    setRoomAction(room.id);
  ...
  function getFilterLabel(f: string): string {
    switch (f) {
      case "all":       return t.admin.filterAll;
      case "bilateral": return t.admin.filterBilateral;
      case "confirmed": return t.admin.confirmed;
      case "pending":   return t.admin.pending;
      case "proposed":  return t.admin.proposed;
  ...
  function applyFilter(list: Reservation[]) {
    ...
    switch (filter) {
      case "bilateral": return list.filter(r => r.type === "bilateral");
      case "confirmed": return list.filter(r => r.status === "confirmed");
      case "pending":   return list.filter(r => r.status === "pending");
      case "proposed":  return list.filter(r => r.status === "proposed");
  ...
  return (
    <div className="flex flex-col gap-[10px]">
        ...
        <div className="flex items-center gap-2">
          <div ref={downloadRef} className="relative">
            ...
            >
              ...
              {downloading === "excel" ? "Excel…" : downloading === "pdf" ? "PDF…" : t.admin.downloadReport}
            ...
            {showDownload && (
              <div className="absolute right-0 top-full mt-1.5 w-52 bg-card border border-border rounded-xl shadow-xl z-50 overflow-hidden animate-slide-up">
                <div className="px-3 py-2 border-b border-border">
                  <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">{t.admin.exportFormat}</p>
                </div>
                <button onClick={handleDownloadExcel} className="w-full flex items-center gap-3 px-3 py-3 text-sm hover:bg-muted/60 transition-colors text-left">
                  ...
                  <div>
                    <p className="font-medium text-foreground">{t.admin.excelReport}</p>
                    <p className="text-[11px] text-muted-foreground">{t.admin.excelDesc}</p>
                  </div>
                </button>
                <button onClick={handleDownloadPDF} className="w-full flex items-center gap-3 px-3 py-3 text-sm hover:bg-muted/60 transition-colors text-left border-t border-border/60">
                  ...
                  <div>
                    <p className="font-medium text-foreground">{t.admin.pdfReport}</p>
                    <p className="text-[11px] text-muted-foreground">{t.admin.pdfDesc}</p>
                  </div>
                </button>
              </div>
            )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-[10px]">
        {[
          { label: t.admin.totalRes,    value: stats.total,     icon: CalendarDays,  color: "text-foreground",   bg: "bg-muted" },
          { label: t.admin.confirmed,   value: stats.confirmed, icon: CheckCircle2,  color: "text-brand",        bg: "bg-brand-light" },
          { label: t.admin.pending,     value: stats.pending,   icon: Clock,         color: "text-olive-dark",   bg: "bg-olive-light" },
          { label: t.admin.proposed,    value: stats.proposed,  icon: CalendarClock, color: "text-blue-600",     bg: "bg-blue-50" },
          { label: t.admin.cancelled,   value: stats.cancelled, icon: XCircle,       color: "text-red-600",      bg: "bg-red-50" },
          { label: t.admin.bilateral,   value: stats.bilateral, icon: Users,         color: "text-brand",        bg: "bg-brand-light" },
          { label: t.admin.bookedRooms, value: stats.salle,     icon: DoorOpen,      color: "text-olive-dark",   bg: "bg-olive-light" },
          { label: t.admin.users,       value: users.length,    icon: TrendingUp,    color: "text-purple-600",   bg: "bg-purple-50" },
      ...
      <div className="flex flex-wrap gap-2">
        {(["reservations", "users", "rooms"] as const).map(tabKey => (
          <button key={tabKey} onClick={() => setTab(tabKey)}
            className={cn("px-5 py-2 rounded-lg text-sm font-medium border transition-colors",
              tab === tabKey ? "bg-brand text-white border-brand" : "bg-card border-border text-muted-foreground hover:border-brand/40"
            )}>
            {tabKey === "reservations" ? `${t.admin.tabReservations} (${reservations.length})`
              : tabKey === "users" ? `${t.admin.tabUsers} (${users.length})`
              : `${t.admin.tabRooms} (${rooms.length})`}
          </button>
        ))}
      </div>
        ...
        <>
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {(["confirmed", "pending", "proposed", "cancelled"] as const).map(f => (
                <button key={f} onClick={() => setFilter(f)}
                  className={cn("px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors",
                    filter === f ? "bg-brand text-white border-brand" : "bg-card border-border text-muted-foreground hover:border-brand/40"
                  )}>
            ...
            <table className="w-full text-sm min-w-[700px]">
              <thead>
                <tr className="border-b border-border bg-muted/40">
                  {[t.admin.colType, t.admin.colTitle, t.admin.colInitiator, t.admin.colGuestRoom, t.admin.colDate, t.admin.colSchedule, t.admin.colStatus, t.admin.colActions].map(h => (
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={8} className="text-center py-12 text-muted-foreground text-sm">{t.admin.noReservations}</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {tab === "users" && (
        <div className="bg-card rounded-2xl border border-border overflow-x-auto">
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                {["", t.admin.displayNameLabel, "Email", t.admin.tabReservations, "Date", t.admin.colActions].map(h => (
            ...
            <tbody>
              ...
              {users.length === 0 && (
                <tr><td colSpan={6} className="text-center py-12 text-muted-foreground text-sm">{t.admin.noUsers}</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {tab === "rooms" && (
        <>
          <div className="flex justify-end">
            ...
            >
              <Plus className="w-4 h-4" /> {t.admin.newRoom}
            </button>
          </div>
          ...
          <div className="bg-card rounded-2xl border border-border overflow-x-auto">
            <table className="w-full text-sm min-w-[550px]">
              <thead>
                <tr className="border-b border-border bg-muted/40">
                  {[t.admin.roomNameLabel, t.admin.floorLabel, t.admin.capacityLabel, t.admin.equipmentLabel, t.admin.colStatus, t.admin.colActions].map(h => (
                    <th key={h} className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={room.id} className={cn("hover:bg-muted/30 transition-colors", i < arr.length - 1 && "border-b border-border")}>
                    ...
                    <td className="px-4 py-3">
                      <span className={cn(
                        ...
                        {room.locked ? t.admin.roomLocked : t.admin.available}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      {roomAction === room.id ? (
                ...
                {rooms.length === 0 && (
                  <tr><td colSpan={6} className="text-center py-12 text-muted-foreground text-sm">{t.admin.noRooms}</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-card rounded-2xl border border-border shadow-xl p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-semibold">{t.admin.editUserTitle}</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">{t.admin.displayNameLabel}</label>
                <input
                  value={editName}
      ...
      {roomModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-card rounded-2xl border border-border shadow-xl p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="text-base font-semibold">
                  {roomModal === "create" ? t.admin.newRoom : `${t.admin.save} — ${editRoom?.name}`}
                </h2>
              </div>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">{t.admin.roomNameLabel} *</label>
                <input
                  value={roomForm.name}
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">{t.admin.floorLabel} *</label>
                <input
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">{t.admin.capacityLabel}</label>
                <input
                  type="number"
                  min="0"
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1 block">{t.admin.equipmentLabel}</label>
                <input
                ...
                />
                <p className="text-[11px] text-muted-foreground mt-1">{t.admin.equipmentHint}</p>
              </div>
